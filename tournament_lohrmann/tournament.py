...
def registerPlayer(name):
    ...
    conn = connect()
    cur = conn.cursor()
    playerId = cur.execute("""INSERT INTO players ( playername, numberofwins, numberofmatches) values (%s, 0, 0) """,(name,))
    conn.commit()
    conn.close
...
def playerStandings():
    ...
    conn = connect()
    cur = conn.cursor()
    cur.execute("""SELECT playerid, playername, numberofwins, numberofmatches from players p Group by playerid, playername, numberofwins, numberofmatches ORDER BY numberofwins DESC;""")
    data = cur.fetchall()
    resultList = []
    for dataOut in data:
